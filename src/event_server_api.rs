...
pub fn ping_event_server() -> bool {
    let reqest_body = EventSeverRequest {
...
pub fn get_chunk_of_events(after_sequence_number: i64) -> Option<Vec<Event>> {
    let reqest_body = EventSeverRequest {
        method: "getChunkOfEvents".to_string(),
        parameters: GetChunkOfEventsRequest { afterSequenceNumber: after_sequence_number },
    };
    let reqest_body = json::encode(&reqest_body).unwrap();
...
pub fn get_highest_sequence_number() -> Option<i64> {
    let reqest_body = EventSeverRequest {
        method: "getHighestSequenceNumber".to_string(),
        parameters: "",
    };
    let reqest_body = json::encode(&reqest_body).unwrap();
    ...
    match status {
        hyper::Ok => {
            let result: EventServerResult<SequenceNumberContainer> = json::decode(&response_body)
                .unwrap();
